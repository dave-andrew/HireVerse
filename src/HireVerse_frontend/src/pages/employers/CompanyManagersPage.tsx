...
export default function CompanyManagersPage() {
    ...
    let {
        data: rawCompanyInvitation,
        refetch: getRawCompanyInvitation,
        isFetching
    ...
    const {data: rawManagerData} = getManagersFromCompany(
        selectedCompany?.id,
    );
    ...
    return (
        <>
            <div
                className="absolute z-0 h-96 w-full bg-[url(/backgrounds/liquid-cheese.svg)] bg-cover opacity-50"></div>
            <ManagementPageLayout>
                <div className="z-10 flex flex-col gap-8 px-14 py-8">
                    <div className="flex flex-col gap-4">
                        <div className="mt-8 text-4xl font-bold">Managers</div>
                        <div className="flex flex-col">
                            <div className="font-semibold text-gray-600">These person are responsible for managing job
                                posting in this company.
                            </div>
                            <div className="text-gray-600">
                                Once invited you cannot kick the manager, you may ask them to kindly leave managing the
                                company instead.
                            </div>
                        </div>
                    </div>
                    <div className="flex flex-col gap-4">
                        <div className="flex h-12 flex-row place-items-end justify-end gap-4">
                            <CardLayout className="flex h-full w-96 flex-row items-center bg-white">
                                <span
                                    className="has-[:focus]:ring-signature-primary flex flex-1 flex-row gap-2 rounded-bl-xl rounded-tl-xl p-3  transition-colors has-[:focus]:bg-gray-100 has-[:focus]:ring-2">
                            ...
                            <button
                                ...
                                onClick={toggleModal}>
                                <CardLayout
                                    className="flex h-full place-items-center justify-center rounded-xl px-8 py-4 hover:bg-gray-100">
                                    + Add Manager
                                </CardLayout>
                            </button>
                        </div>
