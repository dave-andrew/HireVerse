...
import { useGetManagersFromCompany } from "../../datas/queries/jobQueries";
...
export default function ManagerTable() {
    ...
    const { data: managerData, refetch: getJobs } = useGetManagersFromCompany(
        selectedCompany?.id,
    );
                ...
                {managerData.map((manager, index) => {
                    return (
                        <tr
                            ...
                            <td> {(new Date(manager.birth_date)).toLocaleDateString("en-US", {
                                month: "long",
                                day: "numeric",
                                year: "numeric",
                            })}</td>
                            <td className="pr-0 xl:pr-60">{manager.email}</td>
                            <td
                                className={
