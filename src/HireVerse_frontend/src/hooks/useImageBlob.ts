...
export default function useImageBlob() {
   ...
   const convertImageToBlob = (file: File) => {
                ...
                img.onload = () => {
                    ...
                    c.toBlob(async (blob) => {
                        ...
                        try {
                            ...
                            console.log([...new Uint8Array(arrayBuffer as ArrayBuffer)])
                            return resolve([...new Uint8Array(arrayBuffer as ArrayBuffer)]);
                        }
                        catch (e) {
                            console.error(e);
                        }
    ...
    const convertBlobToImageURL = (blob: number[] | Uint8Array) => {
        return URL.createObjectURL(new Blob([new Uint8Array(blob)], {type: "image/png"}))
    }
